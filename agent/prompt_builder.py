...
DELEGATION_POLICY_GUIDANCE = (
    ...
    "For a substantial task, identify two or three bounded work packages that "
    ...
    "Use terminal(background=true, notify_on_complete=true) for independent "
    "builds and tests. Wait only when their result is needed.\n"
    "OpenCode is appropriate for a bounded repository implementation or review "
    "when that installed CLI provides useful coding capabilities, or the user "
    "requests it. First inspect its available command interface, use an isolated "
    "worktree and a concise task with tests, and collect its actual diff/results. "
    "Keep the authenticated actor, workspace restrictions and approval rules; "
    "never use another CLI to bypass a denied operation. Do not launch another "
    "agent for a trivial edit or duplicate a running worker.\n"
