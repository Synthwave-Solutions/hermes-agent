    ...
    * Anything not in ``_PARALLEL_SAFE_TOOLS`` and not an opted-in MCP
    ...
    for tool_call in tool_calls:
        ...
        if tool_name == "terminal":
            if not _terminal_is_parallel_safe(function_args):
                _add_sequential(tool_call)
                continue
        ...
        if tool_name == "process":
            if not _process_is_parallel_safe(function_args):
                _add_sequential(tool_call)
                continue
            current.append(tool_call)
            continue
        ...
        if tool_name == "browser_exec":
            session_key = _browser_session_key(function_args)
        ...
        if tool_name in _SEQUENTIAL_TOOLS or _is_mcp_tool_parallel_blocked(
            tool_name
        ):
            _add_sequential(tool_call)
            continue
